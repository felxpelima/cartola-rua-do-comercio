...
export function extractCartolaRoundSnapshot(payload = {}, scoredPayload = null, matchesPayload = null) {
  ...
  // Enquanto a rodada está EM ANDAMENTO, o `pontos` que o Cartola devolve em
  // /time/id/{id}/{rodada} pode ainda ser o total OFICIAL da rodada ANTERIOR
  // (dado velho), não o desta rodada — foi o que duplicou os pontos. Com a
  // rodada em andamento a fonte confiável é a parcial calculada a partir de
  // /atletas/pontuados (sempre da rodada corrente). Só voltamos a confiar no
  // `pontos` oficial quando a rodada encerrou (ou quando o /partidas não permite
  // afirmar nada — comportamento legado).
  if (roundIsFinished(matchesPayload) === false) {
    return partial;
  }
