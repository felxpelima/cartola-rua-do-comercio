...
function publicConfig(row) {
  ...
  return {
    ...
    mural: row.mural || "",
    temporadaEncerrada: Boolean(row.temporadaEncerrada),
  };
}
...
// Colunas estáveis do AppConfig (tudo menos `bets`). Servem de leitura segura
// quando `bets` ainda não existe no banco: um deploy de código antes do
// `prisma db push` não pode derrubar o estado principal (nomes/escudos/ranking).
const CONFIG_SELECT = {
  id: true,
  titulo: true,
  subtitulo: true,
...
async function getConfig() {
  ...
  try {
    ...
    // A coluna `bets` ainda não existe (deploy entrou antes do db push)? Relê só
    // as colunas estáveis. As apostas saem vazias até a migração rodar, mas o
    // estado moderno continua montando — sem cair no fallback de dados legados.
    const safe = await prisma.appConfig.findUnique({ where: { id: 1 }, select: CONFIG_SELECT });
    if (safe) return safe;
  }
