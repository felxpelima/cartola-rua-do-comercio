...
function buildHighlights(ranking, roundRanking) {
  ...
  addHighlight(highlights, leader && {
    ...
    body: `Esta no topo com ${toNumber(leader.totalPoints).toFixed(2)} pts.`,
  ...
  if (leader && vice) {
    ...
    addHighlight(highlights, {
      ...
      label: "Na cola",
      title: teamName(vice),
      body: `Esta a ${diff.toFixed(2)} pts do lider.`,
  ...
  addHighlight(highlights, bestClimb && {
    ...
    body: `Subiu ${Number(bestClimb.delta)} posicao(oes) no geral.`,
  ...
  addHighlight(highlights, biggestFall && {
    ...
    body: `Caiu ${Math.abs(Number(biggestFall.delta))} posicao(oes). Ainda tem jogo.`,
  ...
  addHighlight(highlights, bestAverage && bestAverage.id !== leader?.id && {
    ...
    body: `Media de ${toNumber(bestAverage.average).toFixed(2)} pts por rodada.`,
...
async function getModernState() {
  ...
  return {
    ...
    mitadas: buildMitadas(ranking, rounds, scores),
    lastSync,
    automation: {
